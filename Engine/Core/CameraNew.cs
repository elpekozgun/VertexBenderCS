...
namespace Engine.Core
{
    public class CameraNew
    {
        ...
        public CameraNew(float aspect)
        {
            ...
            Projection = Matrix4.CreatePerspectiveFieldOfView(MathHelper.PiOver4, aspect, 0.01f, 1000);
            View = CreateLookAt();
        }
        ...
        public CameraNew(float aspect, Vector3 position, Vector3 target) : this(aspect)
        {
            m_position = position;
        ...
        protected virtual void ProcessInput()
        {
            ...
            m_direction = Vector3.Transform(m_direction,
                Matrix3.CreateFromAxisAngle(m_up, -m_mouseSpeedX * (mouse.X - m_prevMouse.X))
            );
            ...
            {
                m_direction = Vector3.Transform(m_direction,
                    Matrix3.CreateFromAxisAngle(Vector3.Cross(m_up, m_direction), angle)
                );
            }
